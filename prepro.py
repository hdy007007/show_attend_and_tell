...
def _process_caption_data(caption_file, image_dir, max_length):
    ...
    for annotation in caption_data:
        ...
        annotation['image_id'] = os.path.join(image_dir, id_to_filename[image_id])
        data += [annotation]
    
    # convert to pandas dataframe (for later visualization or debugging)
    caption_data = pd.DataFrame.from_dict(caption_data)
    del caption_data['url']
...
def main():
    ...
    # vgg model path 
    vgg_model_path = './ai.challenger/imagenet-vgg-verydeep-19.mat'

    #caption_file = './ai.challenger/data/annotations/caption_train_annotations_20170902.json'
    caption_file = '/home/houdanyang/tensorflow/coco-caption/annotations/captions_val2014.json'
    image_dir = './ai.challenger/image/train_images_2017_resized/'
    ...
    for split in ['train', 'val', 'test']:
        ...
        if split == 'train':
            ...
            load_pickle( './data/%s/word_to_idx.pkl' % split)
        #captions = _build_caption_vector(annotations=annotations, word_to_idx=word_to_idx, max_length=max_length)
        #save_pickle(captions, './data/%s/%s.captions.pkl' % (split, split))
        load_pickle( './data/%s/%s.captions.pkl' % (split, split))
