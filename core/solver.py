...
class CaptioningSolver(object):
    def __init__(self, model, val_data, **kwargs):
        """
        Required Arguments:
        ...
        with tf.Session(config=config) as sess:
            ...
            for e in range(self.n_epochs):
                for part_num in range(5):
                    data = load_coco_data(data_path='./data', split='train',batch = part_num)
                    ...
                    for i in range(n_iters_per_epoch):
                        ...
                        if (i+1) % self.print_every == 0:
                            print "\nTrain loss at epoch %d & part %d & iteration %d (mini-batch): %.5f" %(e+1,part_num + 1, i+1, l)
                ...
                if self.print_bleu:
                    ...
                    captions2json.captions2json(all_decoded,self.val_data['file_names'],'./data/val/val_cadidate_captions_json.json')
                    compute_m1(json_predictions_file = './data/val/val_cadidate_captions_json.json', 
                               reference_file = './data/val/val_references_json.json')

                # save model's parameters
                if (e+1) % self.save_every == 0:
           
                    saver.save(sess, os.path.join(self.model_path, 'model.ckpt'),global_step = e + 1)
