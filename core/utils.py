...
def load_coco_data(data_path='./data', split='train', batch = 0):
    data_path = os.path.join(data_path, split)
    start_t = time.time()
    data = {}
    batch_dict = [[0,50000],[50000,100000],[100000,150000],[150000,200000],[200000,209918]]
    if split == 'train': 
        data['features'] = hickle.load(os.path.join(data_path, '%s.features%d.hkl' %(split,batch + 1)))
        with open(os.path.join(data_path, '%s.file.names.pkl' %split), 'rb') as f:
            tt = pickle.load(f)
            data['file_names'] = tt[batch_dict[batch][0]:batch_dict[batch][1]]
        
        with open(os.path.join(data_path, '%s.image.idxs.pkl' %split), 'rb') as f:
            tt = pickle.load(f)
            first_index = np.min(np.where(tt == batch_dict[batch][0]))
            last_index = np.max(np.where(tt == batch_dict[batch][1] - 1))
            data['image_idxs'] = tt[first_index:last_index + 1] - batch_dict[batch][0]
        
        with open(os.path.join(data_path, '%s.captions.pkl' %split), 'rb') as f:
            tt = pickle.load(f)
            data['captions'] = tt[first_index:last_index + 1] 
...
def sample_coco_minibatch(data, batch_size):
    ...
    features = [data['features'][x] for x in mask ]
    #file_names = data['file_names'][mask]
    file_names = [data['file_names'][x] for x in mask]
